...
public class Maze {
	...
	/*
	 * private void placeGoal(char feature, char replace) { // Change this to make
	 * is far away from the player. System.out.println("Place goal.");
	 * 
	 * 
	 * int row = (int) (maze.length * Math.random()); int col = (int)
	 * (maze[0].length * Math.random());
	 * 
	 * //if (maze[row][col].getNodeType() == replace){ //door = new
	 * Node(row,col,feature); maze[row][col].setNodeType(feature);
	 * maze[row][col].setGoalNode(true); maze[row][col].setWalkable(true); goal =
	 * maze[row][col]; System.out.println("Place - ");
	...
	private void generateMaze() { // Binary Maze gen.
		for (int row = 0; row < maze.length; row++) {
			for (int col = 0; col < maze[row].length - 1; col++) {
				/*
				 * int num = (int) (Math.random() * 10); if (col > 0 && (row == 0 || num >= 5)){
				 * maze[row][col].addPath(Node.Direction.West); }else{
	...
	// private boolean isRoom(int row, int col) { // Flaky and only works half the
	// time, but reduces the number of rooms
	// return row > 1 && maze[row - 1][col].getNodeType() == ' ' && maze[row -
	// 1][col + 1].getNodeType() == ' ';
	// }
