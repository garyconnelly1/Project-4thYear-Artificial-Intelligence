	...
	public void resetA(Node[][] maze) {
		for (int i = 0; i < maze.length; i++) {
			for (int j = 0; j < maze[i].length; j++) {
				...
				if (maze[i][j].getNodeType() == 'Q' || maze[i][j].getNodeType() == ' ') {
				//if (maze[i][j].getNodeType() == NodeType.HELPSPIDER || maze[i][j].getNodeType() == NodeType.ROAD) {
					maze[i][j].setNodeType(' ');
					//maze[i][j].setNodeType(NodeType.ROAD);
